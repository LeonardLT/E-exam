...
export default class ReviewExam extends React.Component {
                ...
                request.get("/api/exams/tExam")
                    ...
                    .end((err, res) => {
                        const data = res.body.map(({_id,examState,examType, examName, publishDate, branch, major, classroom}) => {
                            return {_id,examState,examType, examName, publishDate: moment(publishDate).format('YYYY-MM-DD'), branch, major, classroom};
                        });
                        this.setState({
                            examLists: data
                        });
    ...
    _showPaper(examId, userId) {
        return () => {
            hashHistory.push('/ReviewExamSelPaper/' + examId + "&" + userId);
        };
    }
