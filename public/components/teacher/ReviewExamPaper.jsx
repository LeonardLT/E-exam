...
export default class ReviewExamPaper extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            studentSelScore: 0,
            studentName: '',
            selectQuestionsScore: 0,
            shortAnswerQuestionsScore: 0,
    ...
    studentshortAnswerScoreChange(value) {
        if (value >= this.state.shortAnswerQuestionsScore) {
            message.warning("简答题满分" + this.state.shortAnswerQuestionsScore);
            this.setState({
                studentShortAnsScore: this.state.shortAnswerQuestionsScore
            })
        } else {
            this.setState({
                studentShortAnsScore: value
            });
        }
    }
    ...
    _submitScore(e) {
        const _this = this;
        const score = this.state.studentShortAnsScore + this.state.studentExamScore.selScore;
                ...
                <Content style={{padding: '0 50px'}}>
                    ...
                    <Layout style={{padding: '24px 0', background: '#fff'}}>
                        ...
                        <Content style={{padding: '0 24px', minHeight: 800}}>
                            ...
                            选择题分数：<Tag color="green">{this.state.studentSelScore}</Tag>
                            简答题分数：<InputNumber min={0} max={this.state.shortAnswerQuestionsScore} defaultValue={0}
                                               value={this.state.studentShortAnsScore} onChange={this.studentshortAnswerScoreChange.bind(this)}/>
                            &nbsp;
